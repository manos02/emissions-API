...
function CountriesISO() {
  ...
  const componentDidMount = async () => {
    try {
      const response = await CountriesISOService.getCountriesISO(
        params,
        queryParams,
      );
  ...
  async function handlePost() {
    if (
      ...
      try {
        const response = await CountriesISOService.postCountriesISO(
          params,
          formData,
        );
  ...
  return (
    <div>
      ...
      <div className="FORM">
        <a style={{ fontWeight: "bold" }}>CREATE NEW DATA ENTRY</a>
        <form>
          <label>
            Year of new data:
            <input
      ...
      {country.data.map((item, index) => (
        <div key={index} className="ISO">
          <button
            ...
            onClick={() => goRouteISOYear(country.iso, item.year)}
          >
            Year: {item.year}
          </button>
