...
function ContinentsName() {
  ...
  const componentDidMount = async () => {
    try {
      const response = await ContinentsNameService.getContinentsName(
        params,
        queryParams,
      );
  ...
  function handleOptions(item) {
    ...
    } else if (dataTypeForm == 1) {
      return handleEmissionData(item);
    } else if (dataTypeForm == 2) {
      return handleEnergyData(item);
    } else if (dataTypeForm == 3) {
  ...
  async function handlePost(event) {
    if (year !== null && year !== undefined && year !== "" && year >= 0) {
      try {
        const response = await ContinentsNameService.postContinentsName(
          params,
          year,
        );
        setYear(null);
  ...
  return (
    <div>
      ...
      <div className="FORM">
        <a style={{ fontWeight: "bold" }}>CREATE NEW DATA</a>
        <form>
          <label>
            Year of new data:
            <input
      ...
      {continent.data.map((item, index) => (
        <div key={index} className="ISO">
          <button
            ...
            onClick={() => goRouteNameYear(continent.name, item.year)}
          >
            Year: {item.year}
          </button>
          <div className="ISO-data">{handleOptions(item)}</div>
          <hr />
        </div>
      ))}
    </div>
